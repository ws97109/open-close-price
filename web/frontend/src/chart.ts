...
interface PredictionPoint {
  ...
  next_time: number
  last_close: number
  gap_target_price: number
  close_target_price: number
  prob_gap: number
  prob_close: number
  conf_gap: number
  conf_close: number
...
interface ChartPayload {
  ...
  ma_lines: Record<string, LineData[]>
  gap_probs: LineData[]
  close_probs: LineData[]
  prediction_point: PredictionPoint
}

// ─── Chart state ────────────────────────────────────────────────
let _chart:   IChartApi | null = null
let _candles: ISeriesApi<'Candlestick'> | null = null

// ─── Fetch chart data ────────────────────────────────────────────
export async function fetchChartData(stockId: string): Promise<ChartPayload> {
...
// ─── Render chart ────────────────────────────────────────────────
export function renderChart(container: HTMLElement, data: ChartPayload): void {
  ...
  if (data.gap_probs.length > 0) {
    ...
    baseLine.setData(baselineData as any)
  }
  ...
  if (data.gap_probs.length > 0) {
    ...
    gapProbSeries.setData(colored as any)
  }
  ...
  if (data.close_probs.length > 0) {
    ...
    closeProbSeries.setData(data.close_probs as any)
  }
