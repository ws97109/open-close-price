...
function changeCard(ch, closeSignal) {
...
function signalCard(title, accNote, side) {
...
function renderResult(d) {
    ...
    // News list
    const newsHtml = d.sentiment.articles.slice(0, 6).map((a) => {
...
// ─── Ollama analysis ───────────────────────────────────────────
function attachAnalyzeBtn(result) {
    ...
    btn.addEventListener('click', async () => {
        ...
        let full = '';
        await analyzeStream(result.stock, result.stock_name, result, (chunk) => {
            full += chunk;
